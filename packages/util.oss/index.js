// @bun
// packages/util.oss/src/cache/cache.ts
...
// packages/util.oss/src/cache/cache-with-no-cache.ts
...
// packages/util.oss/src/fs-extended.ts
import { Buffer } from "buffer";
import crypto2 from "crypto";
import fs from "fs";
import path from "path";

// packages/util.oss/src/fs-extended.mime-type-map.ts
var mimeTypeMap = {
...
// packages/util.oss/src/fs-extended.ts
var fse = {
  isExists: (path2) => fs.promises.access(path2).then(() => true).catch(() => false),
  isFile: (path2) => fse.stat(path2).then((stats) => stats.isFile()).catch(() => false),
  isDir: (path2) => fse.stat(path2).then((stats) => stats.isDirectory()).catch(() => false),
  isImage: (path2) => fse.mimeType.lookup(path2).startsWith("image/"),
  stat: (path2) => fs.promises.stat(path2),
  ...
  name: {
    findUnique: async (inDir, name) => {
      ...
      let newName = name;
      let count = 1;
      while (await fse.isExists(path.resolve(inDir, newName)))
  ...
  path: {
    join: (...args) => path.join(...args).replaceAll("\\", "/"),
    resolve: (...args) => path.resolve(...args).replaceAll("\\", "/")
  },
  mimeType: {
    map: mimeTypeMap,
    lookup: (filename) => {
...
class TmpFileCreator {
  ...
  get targetDir() {
    let targetDir = path.join(this.path, crypto2.randomUUID());
...
// packages/util.oss/src/password.ts
import * as crypto3 from "crypto";
async function hash(string, algorithm = "sha256", encoding = "hex") {
  return crypto3.createHash(algorithm).update(string).digest(encoding);
}
...
class Jwt {
  ...
  decode(token) {
    ...
    let payload = jwt.verify(token, this.secret, { algorithms: [this.algorithm] });
