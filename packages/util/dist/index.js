...
class TmpFile {
  ...
  async release() {
    if (await fileExists(this.file))
      await fs2.promises.unlink(this.file).then(() => fs2.promises.rmdir(path.dirname(this.file)));
  }
}
...
class TmpFileFactory {
  ...
  get targetDir() {
    ...
    return fs2.promises.mkdir(targetDir).then(() => targetDir);
  }
  async createFromFile(file, removeOriginal = true) {
    return this.createFromBuffer(file.name, Buffer.from(await file.arrayBuffer()));
  }
  async createFromBuffer(filename, buffer) {
    let target = path.join(await this.targetDir, path.basename(filename));
    await fs2.promises.writeFile(target, buffer);
    return new TmpFile(target);
  }
  async createFromFilePath(file, removeOriginal = true) {
    let target = path.join(await this.targetDir, path.basename(file));
    if (removeOriginal)
      await fs2.promises.rename(file, target);
    else
      await fs2.promises.copyFile(file, target);
    return new TmpFile(target);
  }
}
// src/fs/get-unique-filename.ts
import Path from "path";
async function getUniqueFilename(inDir, filename) {
  const baseName = Path.basename(filename, Path.extname(filename));
  const extName = Path.extname(filename);
  let newName = filename;
  let count = 1;
  while (await fileExists(Path.resolve(inDir, newName)))
...
// src/fs/remove-empty-parent-directories.ts
import * as fs3 from "fs";
import * as path2 from "path";
async function removeEmptyParentDirectories(dir) {
  try {
    const parent = path2.parse(dir).dir;
    const list = await fs3.promises.readdir(dir);
    if (list.length === 0) {
...
class Cache {
  ...
  constructor(ttl, prefix) {
    ...
    if (this.prefix === undefined)
      this.prefix = crypto2.randomUUID();
  }
  getReader(ttl) {
    const _ttl = ttl === undefined ? this.ttl : ttl;
    return (handler, key, ttl2 = _ttl) => {
      return this.read(handler, key, ttl2);
    };
  }
  getInvalidator() {
...
// src/cache/cache-with-no-cache.ts
class CacheWithNoCache extends Cache {
  constructor(ttl, prefix) {
    super(ttl, prefix);
  }
  get(key) {
...
// src/fs-extended.ts
var fse = {
  isExists: (path4) => fs4.promises.access(path4).then(() => true).catch(() => false),
  isFile: (path4) => fse.stat(path4).then((stats) => stats.isFile()).catch(() => false),
  isDir: (path4) => fse.stat(path4).then((stats) => stats.isDirectory()).catch(() => false),
  isImage: (path4) => fse.mimeType.lookup(path4).startsWith("image/"),
  stat: (path4) => fs4.promises.stat(path4),
  fileSize: (file) => fse.stat(file).then((stat) => stat !== null ? stat.size : 0),
  name: {
    findUnique: async (inDir, name) => {
      const baseName = path3.basename(name, path3.extname(name));
      const extName = path3.extname(name);
      let newName = name;
      let count = 1;
      while (await fse.isExists(path3.resolve(inDir, newName)))
        newName = `${baseName}(${count++})${extName}`;
      return newName;
    },
    sanitize: (name) => name.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9_.]/g, "-").replace(/(^[-.]+)|([-._]+)$/g, "").replace(/-+/g, "-").replace(/\.[-]/g, ".").replace(/[-.]\./g, ".")
  },
  path: {
    join: (...args) => path3.join(...args).replaceAll("\\", "/"),
    resolve: (...args) => path3.resolve(...args).replaceAll("\\", "/")
  },
  mimeType: {
    map: mimeTypeMap2,
    lookup: (filename) => {
      let ext = path3.parse(filename).ext;
      return mimeTypeMap2[ext] === undefined ? "application/octet-stream" : mimeTypeMap2[ext];
    }
  },
  rmEmptyParents: (dir) => {
    const parent = path3.parse(dir).dir;
    fs4.promises.readdir(dir).then((list) => list.length === 0 ? fs4.promises.rmdir(dir).then(() => fse.rmEmptyParents(parent)) : null).catch((error) => console.error(`Error removing directory: ${dir}`, error));
  },
  tmpFileCreatorFactory: (tmp) => new TmpFileCreator(tmp)
};

class TmpFile2 {
  file;
  get filename() {
    return path3.basename(this.file);
  }
  constructor(file) {
    this.file = file;
  }
  async release() {
    if (await fse.isFile(this.file))
      fs4.promises.unlink(this.file).then(() => fs4.promises.rmdir(path3.dirname(this.file)));
  }
}
...
class TmpFileCreator {
  ...
  get targetDir() {
    let targetDir = path3.join(this.path, crypto3.randomUUID());
    return fs4.promises.mkdir(targetDir).then(() => targetDir);
  }
  async createFromFile(file, removeOriginal = true) {
    return this.createFromBuffer(file.name, Buffer2.from(await file.arrayBuffer()));
  }
  async createFromBuffer(filename, buffer) {
    let target = path3.join(await this.targetDir, path3.basename(filename));
    await fs4.promises.writeFile(target, buffer);
    return new TmpFile2(target);
  }
  async createFromFilePath(file, removeOriginal = true) {
    let target = path3.join(await this.targetDir, path3.basename(file));
    if (removeOriginal)
      await fs4.promises.rename(file, target);
    else
      await fs4.promises.copyFile(file, target);
    return new TmpFile2(target);
  }
}
// src/password.ts
