import {firstOrUndefined, MaterializeIt, type State} from "@nano-forge/util";
import {type Cache, fse} from "@nano-forge/util.oss";
...
export class Storage {
	...
	async add(name: string, id: number, file: ITmpFile, metadata: Record<string, any>) {
		...
		filename = fse.name.sanitize(filename);
		filename = await fse.name.findUnique(path, filename);
		...
		attachments.push({
			name: filename,
			size: await fse.fileSize(file.file),
			id: this.now(),
			metadata
		});
		if (res.found === false) {
	...
	async rename(name: string, id: number, filename: string, newName: string) {
		...
		if (idx === -1) throw storageError.attachedFileNotFound(name, id, filename);
		let path = this.getPath(name, id);
		newName = fse.name.sanitize(newName);
		newName = await fse.name.findUnique(path, newName);
	...
	now(): string { return Date.now().toString(36);}
}