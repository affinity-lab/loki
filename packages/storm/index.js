...
// packages/storm/src/core/entity/entity.ts
import { MaterializeIt as MaterializeIt2 } from "@nano-forge/util";

// packages/storm/src/core/helper.ts
import { MetadataLibrary } from "@nano-forge/util";
...
// packages/storm/src/core/entity/view-entity.ts
import { MaterializeIt, omitFieldsIP, pickFieldsIP } from "@nano-forge/util";

// packages/storm/src/core/error.ts
...
// packages/storm/src/core/entity/entity-repository.ts
import { omitFieldsIP as omitFieldsIP2, pickFieldsIP as pickFieldsIP2, ProcessPipeline as ProcessPipeline2 } from "@nano-forge/util";
import { sql as sql3 } from "drizzle-orm";

// packages/storm/src/core/entity/view-entity-repository.ts
import { firstOrUndefined, MaterializeIt as MaterializeIt3, ProcessPipeline } from "@nano-forge/util";
...
// packages/storm/src/extensions/plugin.storage/attachment.ts
import fs from "fs";
...
// packages/storm/src/extensions/plugin.storage/collection.ts
import { bytes } from "@nano-forge/util";
import { fse as fse2 } from "@nano-forge/util.oss";
import * as fs2 from "fs";
import { minimatch as minimatch2 } from "minimatch";
import * as path2 from "path";

// packages/storm/src/extensions/plugin.storage/collection-handler.ts
...
// packages/storm/src/extensions/plugin.storage/helper/error.ts
import { err as err2, errorGroup as errorGroup2 } from "@nano-forge/util";
var storageError = {
...
class Collection {
  ...
  constructor(name, groupDefinition, rules) {
    ...
    if (this.rules.mime !== undefined) {
      ...
      for (const mime of this.rules.mime) {
        for (const ext in fse2.mimeType.map) {
          if (minimatch2(fse2.mimeType.map[ext], mime))
  ...
  async prepare(collectionHandler, file) {
    let metadata;
    const ext = path2.extname(file.filename);
    const filename = path2.basename(file.filename);
...
// packages/storm/src/extensions/plugin.storage/storage.ts
import { firstOrUndefined as firstOrUndefined2, MaterializeIt as MaterializeIt4 } from "@nano-forge/util";
import { fse as fse3 } from "@nano-forge/util.oss";
...
class Storage {
  ...
  cache;
  cleanup;
  constructor(path3, db, schema, cache, cleanup) {
    this.path = path3;
    this.db = db;
    this.schema = schema;
    this.cache = cache;
  ...
  get stmt_get() {
    return stmt(this.db.select().from(this.schema).where(and(sql4`name
            =
            ${sql4.placeholder("name")}`, sql4`itemId
            =
            ${sql4.placeholder("id")}`)).limit(1), firstOrUndefined2);
  }
  get stmt_all() {
    return stmt(this.db.select().from(this.schema).where(and(sql4`itemId
            IN (
            ${sql4.placeholder("ids")}
            )`, sql4`name
            =
            ${sql4.placeholder("name")}`)));
  }
  get stmt_del() {
    return stmt(this.db.delete(this.schema).where(and(sql4`itemId
            = (
            ${sql4.placeholder("id")}
            )`, sql4`name
            =
  ...
  getPath(name, id) {
    return Path.resolve(this.path, name, id.toString(36).padStart(6, "0").match(/.{1,2}/g).join("/"));
  }
  getCacheKey(name, id) {
    return `${name}-${id}`;
  }
  ...
  async destroyFiles(name, id) {
    ...
    const path3 = this.getPath(name, id);
    if (await fse3.isExists(path3)) {
      const files = await fs3.promises.readdir(path3);
      files.map(async (file) => {
        await fs3.promises.unlink(Path.join(path3, file));
        if (this.cleanup !== undefined)
          await this.cleanup(name, id, file);
      });
      await fse3.rmEmptyParents(path3);
    }
  }
  ...
  async add(name, id, file, metadata) {
    let path3 = this.getPath(name, id);
    let filename = Path.basename(file.filename);
    filename = fse3.name.sanitize(filename);
    filename = await fse3.name.findUnique(path3, filename);
    await fs3.promises.mkdir(path3, { recursive: true });
    await fs3.promises.copyFile(file.file, Path.join(path3, filename));
    let res = { found: false };
    ...
    if (res.found === false) {
      ...
      await this.db.update(this.schema).set({ data: JSON.stringify(attachments) }).where(and(sql4`name
            =
            ${sql4.placeholder("name")}`, sql4`itemId
            =
            ${sql4.placeholder("id")}`)).execute({ name, id });
      this.cache?.del(this.getCacheKey(name, id));
    }
    await file.release();
  }
  async delete(name, id, filename) {
    ...
    const path3 = this.getPath(name, id);
    await fs3.promises.unlink(Path.resolve(path3, filename));
    await fse3.rmEmptyParents(path3);
  }
  ...
  async rename(name, id, filename, newName) {
    ...
    if (idx === -1)
      throw storageError.attachedFileNotFound(name, id, filename);
    let path3 = this.getPath(name, id);
    newName = fse3.name.sanitize(newName);
    newName = await fse3.name.findUnique(path3, newName);
    attachments[idx].name = newName;
    await fs3.promises.rename(Path.join(path3, filename), Path.join(path3, newName));
    await this.updateRecord(name, id, attachments);
  }
  plugin() {
    return (repository) => {
...
// packages/storm/src/extensions/plugin.storage/helper/schema-helpers.ts
...
// packages/storm/src/extensions/plugin.storage/helper/storage-plugin.ts
...
// packages/storm/src/extensions/plugin.storage/image-collection.ts
class ImageCollection extends Collection {
  defaultCropMode;
  imgFactory;
  faceDetect;
  constructor(name, groupDefinition, rules, defaultCropMode = "attention", imgFactory, faceDetect = false) {
    super(name, groupDefinition, rules);
    this.defaultCropMode = defaultCropMode;
    this.imgFactory = imgFactory;
    this.faceDetect = faceDetect;
    if (this.rules.ext === undefined)
      this.rules.ext = [".png", ".webp", ".gif", ".jpg", ".jpeg", ".tiff"];
  }
  async prepareFile(file) {
    let metadata = await this.imgFactory(file.file).getInfo(this.faceDetect);
    metadata.crop = this.defaultCropMode;
    return { file, metadata };
  }
}
// packages/storm/src/extensions/plugin.cache/cache-plugin.ts
...
// packages/storm/src/extensions/plugin.cache/result-cache-factory.ts
class ResultCache {
  cache;
  constructor(cache) {
    this.cache = cache;
...
// packages/storm/src/extensions/plugin.cache/cached-get-by-factory.ts
...
// packages/storm/src/extensions/plugin.sequence/sequence-manager.ts
import { omitFieldsIP as omitFieldsIP3 } from "@nano-forge/util";
...
// packages/storm/src/extensions/plugin.tag/group-tag-repository.ts
import { MaterializeIt as MaterializeIt6 } from "@nano-forge/util";
import { and as and4, eq as eq4, not as not2, sql as sql7 } from "drizzle-orm";

// packages/storm/src/extensions/plugin.tag/helper/error.ts
import { err as err3, errorGroup as errorGroup3 } from "@nano-forge/util";
var tagError = {
...
// packages/storm/src/extensions/plugin.tag/tag-repository.ts
import { MaterializeIt as MaterializeIt5 } from "@nano-forge/util";
...
// packages/storm/src/extensions/plugin.tag/helper/schema-helpers.ts
...
// packages/storm/src/extensions/plugin.validator/validator.ts
